...
export function enhanceVideo(video: HTMLVideoElement): void {
  ...
  ctrl.innerHTML = `
    <button type="button" class="${CSS_PREFIX}video-btn ${CSS_PREFIX}speed-btn" title="Click to cycle speed" aria-label="ความเร็ววิดีโอ">1.0x</button>
    <button type="button" class="${CSS_PREFIX}video-btn ${CSS_PREFIX}pip-btn" title="Picture-in-Picture" aria-label="Picture-in-Picture">
